...
- **Absolute paths, not `cd`.** 26% of all Bash calls opened with a `cd` clause. The harness *does* persist cwd between calls — which is exactly the problem: a `cd` silently retargets every later relative path in the session (including parallel calls you batched), and a compound `cd … && …` can trigger a permission prompt a bare command would not. Use absolute paths, or `git -C <dir>` / `uv run --project <dir>`. `cd` only when a command genuinely requires the cwd (some `uv`/`npm` invocations do).
