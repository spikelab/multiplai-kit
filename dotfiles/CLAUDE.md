...
- **Each `WebFetch` failure has its own move — read the status before you re-fetch.** It fails 14.5% of the time in the measured window, 483 of those a bare 403; that is a bot wall, and re-fetching the same URL never clears it. **403/429:** if the `multiplai-media` pack is installed, `/multiplai-media:host-browser` fetches it with the real browser; otherwise drop the URL and say so. **404:** fix the URL. **303:** re-call with the redirect target. **5xx, DNS failure, timeout:** retry verbatim — this is the one case where that is right. **200 with an empty or skeletal body:** also a failure — the page is client-rendered and `WebFetch` cannot run its JS; same remedy as 403.
