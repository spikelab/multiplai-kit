...
- **A failed `WebFetch` is a signal to change something — never retry the same URL verbatim.** `WebFetch` fails 14.5% of the time in the measured window, 483 of those a bare 403; that is a bot wall, and re-fetching the same URL never clears it. On 403/429, drop the URL and say so; on 404 fix the URL; on 303 re-call with the redirect target.
